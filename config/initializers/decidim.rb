...
Decidim.configure do |config|
  ...
  if Rails.application.secrets.maps.present? && Rails.application.secrets.maps[:static_provider].present?
    ...
    config.show_my_location_button = [:proposals]
    config.maps = {
...
# Decidim::Accountability no longer includes ActiveSupport::Configurable,
# so `.configure` is undefined and raises an error.
